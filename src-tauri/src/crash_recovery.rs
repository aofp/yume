...
impl CrashRecoveryManager {
    pub fn new() -> Self {
        ...
        let recovery_state =
            Self::load_recovery_state(&state_path).unwrap_or_else(|_| CrashRecoveryState {
    ...
    fn get_recovery_dir() -> PathBuf {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            PathBuf::from(appdata).join(APP_ID).join("recovery")
        }
    ...
    pub fn recover_session<R: Runtime>(
        ...
        app_handle
            .emit("session-recovered", &snapshot)
    ...
    pub fn record_crash(&self, session_id: Option<String>) {
        ...
        error!(
            "Application crash recorded. Total crashes: {}",
            state.crash_count
        );
