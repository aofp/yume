...
/// Checks multiple locations and returns the best available installation
pub fn find_claude_binary() -> Result<String, String> {
    info!("Searching for claude binary...");

    // Use discover_claude_installations which already sorts by preference
    ...
    // The first one is already the best (sorted by version and source preference)
    if let Some(best) = installations.into_iter().next() {
        // Prefer full paths over bare "claude" for reliability
        let selected = if best.path == "claude" {
            // If we only have "claude" from PATH, try to find a full path alternative
            if let Some(which_result) = try_which_command() {
                if which_result.path != "claude" {
                    info!(
                        "Preferring full path from which: path={}, version={:?}, source={}",
                        which_result.path, which_result.version, which_result.source
                    );
                    which_result
                } else {
                    best
