use crate::db::{Analytics, Database, Message, Session};
...
#[tauri::command]
pub async fn db_save_session(state: State<'_, AppState>, session: Session) -> Result<(), String> {
    let db = state
        .database()
...
#[tauri::command]
pub async fn db_load_all_sessions(state: State<'_, AppState>) -> Result<Vec<Session>, String> {
    let db = state
        .database()
        .ok_or_else(|| "Database not initialized".to_string())?;
...
#[tauri::command]
pub async fn db_save_message(state: State<'_, AppState>, message: Message) -> Result<(), String> {
    let db = state
        .database()
        .ok_or_else(|| "Database not initialized".to_string())?;
...
#[tauri::command]
pub async fn db_get_statistics(state: State<'_, AppState>) -> Result<serde_json::Value, String> {
    let db = state
        .database()
        .ok_or_else(|| "Database not initialized".to_string())?;
...
#[tauri::command]
pub async fn db_clear_all_data(state: State<'_, AppState>, confirm: bool) -> Result<(), String> {
    if !confirm {
...
#[tauri::command]
pub async fn db_export_data(state: State<'_, AppState>) -> Result<serde_json::Value, String> {
    let db = state
        .database()
        .ok_or_else(|| "Database not initialized".to_string())?;
    ...
    for session in sessions {
        ...
        let analytics = db
            .get_session_analytics(&session.id)
