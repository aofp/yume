...
function checkWslAvailable() {
  ...
  try {
    ...
    const result = execSync('wsl --list', {
      encoding: 'utf8',
      stdio: 'pipe',
      windowsHide: true
    });
    ...
    console.log('⚠️ WSL not available:', e.message);
    return false;
  }
}

const WSL_AVAILABLE = checkWslAvailable();
...
function createWslClaudeCommand(args, workingDir) {
  ...
  // On Windows, use full path to wsl.exe
  return ['C:\\Windows\\System32\\wsl.exe', ['-e', 'bash', '-c', findClaudeScript]];
}
...
if (isWindows) {
  ...
  if (WSL_AVAILABLE) {
    ...
    console.log('⚠️ Windows detected but WSL is not available. Claude CLI requires WSL on Windows.');
    CLAUDE_PATH = 'WSL_CLAUDE'; // Still mark it, will show error later
  }
...
task: reply with ONLY 1-3 words describing what user wants. lowercase only. no punctuation. be extremely concise. examples: "echo command", "file search", "debug issue"`;
    ...
    const child = isWindows && CLAUDE_PATH === 'WSL_CLAUDE' ? 
      (() => {
        if (!WSL_AVAILABLE) {
          // Return a dummy process that immediately fails
          ...
          dummy.kill = () => {};
          setTimeout(() => {
            dummy.stderr.emit('data', 'WSL not available');
            dummy.emit('close', 1);
          }, 0);
          return dummy;
        }
        ...
        return spawn(wslCommand, wslArgs, {
          ...
          shell: true,  // Use shell for WSL on Windows
          windowsHide: true,
          detached: false
        });
      })() :
  ...
  socket.on('sendMessage', async (data, callback) => {
    ...
    const spawnRequest = async () => {
      ...
      const claudeProcess = isWindows && CLAUDE_PATH === 'WSL_CLAUDE' ? 
        (() => {
          if (!WSL_AVAILABLE) {
            console.error('❌ WSL is not available on this system. Please install WSL or Claude CLI for Windows.');
            // Create a fake process that immediately errors
            const fakeProcess = new (require('events').EventEmitter)();
            fakeProcess.stdin = { write: () => {}, end: () => {} };
            fakeProcess.stdout = new (require('events').EventEmitter)();
            fakeProcess.stderr = new (require('events').EventEmitter)();
            fakeProcess.pid = -1;
            fakeProcess.kill = () => {};
            
            setTimeout(() => {
              const error = new Error('WSL is not installed. Please install WSL and Claude CLI in WSL, or install Claude CLI for Windows.');
              socket.emit(`error:${sessionId}`, {
                error: error.message,
                sessionId,
                timestamp: Date.now()
              });
          ...
          console.log(`🚀 Running WSL: ${wslCommand}`);
