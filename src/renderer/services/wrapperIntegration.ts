...
export function processWrapperMessage(message: any, sessionId: string): any {
  ...
  if (message.usage && !message.wrapper_tokens) {
    ...
    // Cache read from API - this is part of context window
    session.cacheReadTokens = cacheRead;

    // CORRECT FORMULA for context window (SNAPSHOT, not accumulating):
    // Context = cacheRead + cacheCreation + input
    // This matches the server formula at server-claude-windows.cjs line 210
    // Output tokens are GENERATED by the model, not part of input context window
