console.log('════════════════════════════════════════════════════════════════════');
console.log('🐧 YURUCODE SERVER: Linux External Server (server-claude-linux.cjs)');
console.log('🐧 Platform-specific server for Linux - aligned with macOS/Windows flow');
console.log('🐧 Edit code at: server-claude-linux.cjs');
console.log('🐧 Uses external file for easier debugging and updates');
console.log('════════════════════════════════════════════════════════════════════');

/**
 * Linux-compatible server that runs claude CLI directly
 * Aligned with macOS/Windows servers - NO SDK, NO API KEY - just direct claude CLI calls with streaming
 */
...
function forceKillAllChildren() {
  ...
  try {
    ...
    // pkill may fail if no processes found
  }
}
...
function gracefulShutdown(signal) {
  ...
  // 8. Wait 500ms for SIGTERM to take effect, then SIGKILL any survivors
  setTimeout(() => {
...
Format as a clear, structured summary that preserves all important context.`;
  ...
  socket.on('interrupt', ({ sessionId }, callback) => {
    ...
    if (bashProcess) {
      ...
      try {
        // Kill the bash process - use process group on Linux
        if (bashProcess.pid) {
          try {
